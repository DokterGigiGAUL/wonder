document.addEventListener("DOMContentLoaded", async () => {
  ...
  const isUnlocked =
    !currentComic.premium ||
    (typeof Storage !== "undefined" &&
      Storage.isUnlocked(currentComic.productId));
  ...
  if (!isUnlocked) {
    ...
    if (lock) {
      ...
      if (priceEl) {
        priceEl.textContent =
          `Rp ${currentComic.price.toLocaleString("id-ID")}`;
      }

      const btn = document.getElementById("premiumLockBtn");

      if (btn) {
        btn.href = currentComic.mayarUrl || "#";
      }
    }
  ...
  const response = await fetch(
    `assets/metadata/comic/${currentComic.file}.json`
  );
    ...
    prevButton.addEventListener("click", () => {
      if (currentIndex > 0) {
        window.location.href =
          `comic.html?id=${comics[currentIndex - 1].id}`;
      }
    });
  }
