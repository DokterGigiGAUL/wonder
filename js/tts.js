class CrosswordEngine {
  ...
  async load() {
    try {
      ...
      const hintUnlockedParam = params.get("hintUnlocked");
      if (hintUnlockedParam && typeof Storage !== "undefined") {
        Storage.unlock(`hint_${hintUnlockedParam}`);
      }

      const loader = document.getElementById("loader");

      const isPuzzleUnlocked = !metadata.premium || (typeof Storage !== "undefined" && Storage.isUnlocked(metadata.productId));

      if (!isPuzzleUnlocked) {
        this.showPuzzleLock(metadata, loader);
        return;
      }

      this.hintUnlocked = metadata.premium || (typeof Storage !== "undefined" && Storage.isUnlocked(`hint_${metadata.productId}`));
      this.hintMayarUrl = metadata.hintMayarUrl;
  ...
  showPuzzleLock(metadata, loader) {
    ...
    if (lock) {
      ...
      if (priceEl) priceEl.textContent = `Rp ${metadata.price.toLocaleString("id-ID")}`;
      const btn = document.getElementById("premiumLockBtn");
      if (btn) btn.href = metadata.mayarUrl || "#";
    }
  }
  ...
  showHintLockModal() {
    const overlay = document.getElementById("hintLockModal");
    if (!overlay) return;
    const buyBtn = document.getElementById("hintLockBuy");
    if (buyBtn) buyBtn.href = this.hintMayarUrl || "#";
  ...
  bindEvents() {
    ...
    if (hintBtn) {
      this.updateHintButtonState(hintBtn);
      hintBtn.onclick = () => {
        if (this.hintUnlocked) {
          this.useHint();
        } else {
          this.showHintLockModal();
        }
      };
    }
    ...
    if (revealBtn) {
      this.updateHintButtonState(revealBtn);
      revealBtn.onclick = () => {
        if (this.hintUnlocked) {
          this.revealAnswer();
        } else {
    ...
    if (hintLockCancel && hintLockOverlay) {
      hintLockCancel.onclick = () => hintLockOverlay.classList.remove("show");
    }
    if (hintLockOverlay) {
      hintLockOverlay.onclick = (e) => {
        if (e.target === hintLockOverlay) hintLockOverlay.classList.remove("show");
      };
    }
